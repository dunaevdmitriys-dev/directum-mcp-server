...
## Production-паттерны (из Targets/Agile анализа)

### Z1. BeforeDelete handler — валидация перед удалением
Предотвращает каскадные удаления, проверяет зависимости:
```csharp
public override void BeforeDelete(Sungero.Domain.BeforeDeleteEventArgs e)
{
  base.BeforeDelete(e);
  // Проверить наличие связанных записей
  var linkedDeals = Deals.GetAll(d => d.ResponsibleManager.Equals(_obj));
  if (linkedDeals.Any())
    throw AppliedCodeException.Create(Resources.CannotDeleteLinkedManager);
}
```

### Z2. Saved handler — каскадные обновления и async-триггеры
```csharp
public override void Saved(Sungero.Domain.SavedEventArgs e)
{
  base.Saved(e);
  // Триггер async handler для фоновой обработки
  if (_obj.State.Properties.Status.IsChanged)
  {
    var asyncHandler = AsyncHandlers.UpdateRelatedEntities.Create();
    asyncHandler.EntityId = _obj.Id;
    asyncHandler.ExecuteAsync();
  }
}
```

### Z3. Filtering handler — фильтрация по правам
```csharp
public override IQueryable<T> Filtering(IQueryable<T> query, Sungero.Domain.FilteringEventArgs e)
{
  query = base.Filtering(query, e);
  // Показывать только записи текущего подразделения
  var currentEmployee = Employees.Current;
  if (currentEmployee != null && !currentEmployee.IncludedIn(Roles.Administrators))
    query = query.Where(x => Equals(x.Department, currentEmployee.Department));
  return query;
}
```

### Z4. Locks.TryLock/Unlock — конкурентный доступ (расширенный паттерн)
```csharp
// В AsyncHandler или Job — ОБЯЗАТЕЛЬНА блокировка с retry
if (!Locks.TryLock(entity))
{
  args.Retry = true;
  Logger.WithLogger("DirRX.Module").Debug("Entity {0} locked, retrying", entity.Id);
  return;
}
try
{
...
### Z5. Logger.WithLogger("ModuleName") — структурированное логирование
```csharp
// ПРАВИЛЬНО — с контекстом модуля:
Logger.WithLogger("DirRX.Targets").Debug("Processing board {0}, sprint {1}", boardId, sprintId);
Logger.WithLogger("DirRX.Targets").Error("Failed to update sprint: {0}", ex.Message);

// НЕПРАВИЛЬНО — без контекста (невозможно найти в логах):
Logger.Debug("Processing...");
```

### Z6. Structured response (CommonResponse / PublicStructure)
Вместо сырого JSON возвращать типизированные структуры:
```csharp
// ПРАВИЛЬНО — PublicStructure определённая в Module.mtd:
[Public(WebApiRequestType = RequestType.Get)]
public Structures.Module.ICommonResponse GetDealInfo(long dealId)
{
...
// НЕПРАВИЛЬНО — raw JSON string:
[Public(WebApiRequestType = RequestType.Get)]
public string GetDealInfo(long dealId)
{
  return $"{{\"success\": true, \"data\": ...}}";
}
```

### Z7. ExternalLink API — предопределённые записи
```csharp
// Создание ExternalLink при инициализации (ModuleInitializer)
Sungero.Domain.ModuleInitializer.CreateExternalLink(
  entity, Constants.Module.DefaultBoardExternalLinkId, Constants.Module.ExternalLinkSystemName);

// Поиск по ExternalLink (быстрее GetAll + Where по имени)
var entity = Sungero.Domain.ModuleInitializer.GetEntityByExternalLink<IBoard>(
  Constants.Module.DefaultBoardExternalLinkId, Constants.Module.ExternalLinkSystemName);
```
