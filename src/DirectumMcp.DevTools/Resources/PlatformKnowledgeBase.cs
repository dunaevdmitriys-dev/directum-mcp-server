...
public class PlatformKnowledgeBase
{
    ...
    private const string SolutionsReference = """
        ...
        |--------|---------|-------------|
        | REST API | AgileBoard (30+ endpoints) | MCP: search_metadata — DirRX.AgileBoards/ |
        | Remote Component (React) | Targets (6 RC) | MCP: search_metadata — DirRX.DirectumTargets/ |
        | Kanban-доска | AgileBoard | MCP: search_metadata — DirRX.AgileBoards/ |
        | Email-to-Ticket | ESM (DCS + regex) | MCP: search_metadata — rosa.ESM/ |
        | SLA расчёт | ESM (4 режима) | MCP: search_metadata — rosa.ESM/ |
        | AI-интеграция | ESM (AIAgentTool) | MCP: search_metadata — rosa.ESM/ |
        | XLSX импорт | Targets (Isolated) | MCP: search_metadata — DirRX.KPI/ |
        | Word генерация | Targets (Aspose) | MCP: search_metadata — DirRX.Targets/ |
        | Граф связей many-to-many | AgileBoard (SQL DDL) | MCP: search_metadata — DirRX.AgileBoards/ |
        | Real-time обновления | AgileBoard (ClientManager) | MCP: search_metadata — DirRX.AgileBoards/ |
        ...
        | ExpressionElement | ESM (5 функций) | MCP: search_metadata — rosa.ESM/ |
        | Матричная приоритизация | ESM (Impact×Urgency) | MCP: search_metadata — rosa.ESM/ |
        | Кастомная история | AgileBoard (SQL) | MCP: search_metadata — DirRX.History/ |
        ...
        ### OmniApplied — override модуля
        OmniApplied: override модуля (LayerModuleMetadata), IdentityService, SQL Queries
        Source: omniapplied/source/ (извлечён из .dat)
        """;

    private const string DdsKnownIssues = """
        # 38 известных проблем DDS 25.3/26.1
        ...
        Fix: пересобрать через DDS или вручную:
        ```bash
        resgen ModuleSystem.ru.resx ModuleSystem.ru.resources
        al /embed:ModuleSystem.ru.resources /out:<ModuleName>.Shared.resources.dll
        # Скопировать DLL в контейнер: /app/AppliedModules/ru/
        ...
        ## Ошибки обложки модуля (Cover)

        ### 19. Обложка показывает ключи ресурсов вместо русского текста
        Симптом: вместо "Продажи" отображается CoverTab_<GUID> или пустая строка
        Причина: в cover JSON (таблица sungero_nocode_moduleview) поля Name = null.
        Платформа формирует ключи ресурсов по формуле:
        - Вкладки: CoverTab_<Name>
        - Группы: CoverGroup_<Name>
        - Действия: CoverAction_<Name>
        Без заполненного Name ключ не формируется → ресурс не резолвится.

        Fix (2 шага):
        1. В БД в sungero_nocode_moduleview обновить cover JSON — проставить Name
           для ВСЕХ элементов (Tabs, Groups, Actions). Name берётся из Module.mtd.
           Пример: вкладка "Name": "Sales" → ключ CoverTab_Sales
        2. Убедиться что ModuleSystem.ru.resx содержит ВСЕ ключи:
           - CoverTab_Sales = "Продажи"
           - CoverGroup_Deals = "Сделки"
           - CoverAction_OpenDeals = "Сделки"
        3. Пересобрать satellite DLL если ресурсы были изменены:
           ```bash
           resgen ModuleSystem.ru.resx ModuleSystem.ru.resources
           al /embed:ModuleSystem.ru.resources /out:<ModuleName>.Shared.resources.dll
           ```
        4. Скопировать DLL в контейнер RX: /app/AppliedModules/ru/
        5. Перезапустить контейнеры RX

        Важно: Name в cover JSON ДОЛЖЕН точно совпадать с суффиксом ключа в .resx.
        Если JSON: "Name": "Sales", то resx: CoverTab_Sales = "Продажи".

        ### 20. Обложка модуля пустая (нет вкладок, групп, действий)
        Симптом: модуль открывается, но обложка полностью пустая
        Причина: нет записи moduleview в sungero_nocode_moduleview для данного модуля,
        или cover JSON пустой/невалидный.
        Fix:
        1. Проверить наличие записи: SELECT * FROM sungero_nocode_moduleview WHERE moduleguid = '<GUID>'
        2. Если нет — создать INSERT с cover JSON из Module.mtd (секция Cover)
        3. Проверить что explorermodule, secureobject, modulerec, accessctrlent тоже есть
        4. Статус moduleview должен быть 'Active' (не 'Draft')

        ## Ошибки отображения сущностей

        ### 21. Нет IsDisplayValue → списки показывают "Без имени"
        Симптом: в списках и dropdown вместо названия показывается "Без имени" или GUID
        Причина: ни одно StringPropertyMetadata не имеет "IsDisplayValue": true
        Fix: установить "IsDisplayValue": true ровно на ОДНОМ строковом свойстве (обычно Name)

        ### 22. .resx resheader version mismatch
        Симптом: "Invalid .resx file" или "cannot find valid resheader"
        Причина: resheader version ≠ 2.0, или reader/writer с Version ≠ 4.0.0.0
        Fix: все .resx файлы должны содержать:
        ```xml
        ...
        ### 23. Satellite DLL залочена процессом w3wp.exe / dotnet
        Симптом: изменённые .resx ключи не появляются в UI после замены DLL
        Причина: satellite assembly кэшируется web-сервером, DLL залочена
        Fix: остановить app pool / docker compose stop → заменить DLL → перезапустить

        ### 24. .g.cs перегенерируется пустым при реимпорте
        Симптом: после импорта .dat свойства структур пропадают из сгенерированного кода
        Причина: DDS перегенерирует .g.cs из Module.mtd при каждом открытии. Если Properties пусты — .g.cs пустой
        Fix: восстановить Properties в Module.mtd, перегенерировать .g.cs, установить read-only (attrib +R)

        ## Ошибки производительности

        ### 25. Remote-функции в Showing/Refresh/CanExecute
        Симптом: UI зависает, тысячи запросов к серверу при открытии списка
        Причина: события OnShowing/OnRefresh/CanExecute срабатывают часто (при каждом выделении строки).
        Remote-вызов в них → N запросов на N строк
        Fix: убрать Remote-вызовы из этих обработчиков. Использовать кэширование или предзагрузку

        ### 26. GetAll() без Where → OutOfMemoryException
        Симптом: операция зависает или падает с OutOfMemory на таблицах >100k записей
        Причина: Entities.GetAll().ToList() загружает ВСЕ записи в RAM
        Fix: всегда использовать .Where(...) перед .ToList(). Для больших выборок — чанкование через Transactions.Execute()

        ### 27. N+1 запросов при lazy-load NavigationProperty
        Симптом: выбор 100 записей генерирует 101 SQL-запрос
        Причина: navigation properties загружаются лениво, по одному
        Fix: установить "LoadValueImmediately": true на часто используемых NavigationPropertyMetadata

        ## Ошибки типизации и runtime

        ### 28. NHibernate proxy не проходит C# is/as проверки
        Симптом: `if (entity is IEmployee)` всегда false для proxied объектов
        Причина: NHibernate создаёт runtime proxy классы, которые не проходят C# type checks
        Fix: использовать платформенный API: `Employees.Is(entity)` и `Employees.As(entity)` вместо C# is/as

        ### 29. IsSolutionModule отсутствует в Dependencies
        Симптом: кастомный модуль не появляется в дереве DDS после импорта
        Причина: в Module.mtd Dependencies нет записи с "IsSolutionModule": true
        Fix: добавить {"Id": "<solutionGUID>", "IsSolutionModule": true} в Dependencies

        ### 30. BlockIds с GUID-строками → Invalid C# token
        Симптом: .g.cs содержит синтаксическую ошибку "Invalid token '-'"
        Причина: BlockIds в Task.mtd содержат GUID-строки с дефисами, DDS использует их как C# идентификаторы
        Fix: установить "BlockIds": [] (пустой массив) в Task .mtd

        ### 31. SourceTableName отсутствует в ReportMetadata
        Симптом: обработчик отчёта не компилируется — ссылка на неопределённый Report.SourceTableName
        Причина: .mtd отчёта не содержит PublicConstants с SourceTableName
        Fix: добавить "PublicConstants": [{"Name": "SourceTableName", "Value": "<table_name>"}] в ReportMetadata

        ### 32. SharedNamespace неправильный формат
        Симптом: "Shared namespace does not exist" при компиляции
        Причина: SharedNamespace в Module.mtd должен заканчиваться на .Shared (например DirRX.CRM.Shared)
        Fix: проверить что SharedNamespace точно совпадает: {Company}.{Module}.Shared

        ### 33. Assignment Overridden "DirectValues" вместо "Values"
        Симптом: карточка задания отображается некорректно или модификации Result не работают
        Причина: в Card Form Assignment стоит "Overridden": ["DirectValues"] вместо ["Values"]
        Fix: заменить на "Overridden": ["Values"]

        ### 34. Ancestor GUID не совпадает с платформенным
        Симптом: FilterPanel или CreationArea наследуемой сущности не работают
        Причина: при IsAncestorMetadata: true GUID должен совпадать с платформенным базовым типом.
        Свой GUID ломает наследование
        Fix: использовать фиксированные GUID из reference (например DatabookEntry FilterPanel = b0125fbd-3b91-4dbb-914a-689276216404)

        ### 35. JSON-сериализатор: имена свойств не совпадают со структурой
        Симптом: сериализованный JSON содержит поля, которые не маппятся обратно при десериализации
        Причина: имена свойств в ручном сериализаторе отличаются от Module.mtd PublicStructures
        Fix: имена в сериализаторе ДОЛЖНЫ точно совпадать с именами в Module.mtd → PublicStructures → Properties

        ### 36. sortingpriority в explorermodule — enum, не число
        Симптом: crash при открытии проводника
        Причина: поле sortingpriority в sungero_system_explorermodule принимает enum ('High'/'Medium'/'Low'),
        числовые значения вызывают ошибку
        Fix: использовать строковые значения: 'High', 'Medium', 'Low'

        ### 37. secureobject без entityid/entitytypeguid → crash
        Симптом: ошибка при открытии модуля в проводнике
        Причина: запись в sungero_system_secureobject с пустыми entityid или entitytypeguid
        Fix: ОБЯЗАТЕЛЬНО заполнять entityid и entitytypeguid при создании secureobject

        ### 38. WEB_HOST_HTTP_PORT — общий для всех сервисов RX
        Симптом: один из сервисов RX (IS, WebServer) не стартует или слушает не тот порт
        Причина: все сервисы RX читают WEB_HOST_HTTP_PORT из общего config.xml.
        При изменении порта одного сервиса затрагиваются все
        Fix: проверять порты всех сервисов после изменения config.xml. HAProxy конфиг должен соответствовать
        """;
    ...
    private const string UiCatalog = """
        ...
        ## Production Remote Components (расширенный каталог)
        7 production RC: 6 Targets + 1 OmniApplied Matrix (Application scope)
        metadata.json эталоны: Card, Card+Cover, Application
        """;
    ...
    private const string TargetsPatterns = """
        ...
        ## Source и масштаб
        Source доступен: targets/source/ (извлечён из .dat)
        43 .mtd файла, 500+ .cs, 6 RC с metadata.json
        7 WebAPI endpoints с CommonResponse паттерном
        Версионный Initializer DTCommons (5 версий, ExternalLink)
        """;
}
