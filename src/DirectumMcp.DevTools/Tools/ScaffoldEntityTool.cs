...
public class ScaffoldEntityTool
{
        ...
        [Description("GUID переопределяемой сущности (только для mode=override)")] string ancestorGuid = "")
    {
        ...
        if (!System.Text.RegularExpressions.Regex.IsMatch(entityName, @"^[A-Za-z][A-Za-z0-9_]*$"))
            return "**ОШИБКА**: `entityName` должен содержать только латинские буквы, цифры и подчёркивания, начинаться с буквы.";
        if (!System.Text.RegularExpressions.Regex.IsMatch(moduleName, @"^[A-Za-z][A-Za-z0-9_.]*$"))
            return "**ОШИБКА**: `moduleName` должен содержать только латинские буквы, цифры, точки и подчёркивания.";
