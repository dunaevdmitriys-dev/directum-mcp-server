...
public class ScaffoldModuleServiceTests : IDisposable
{
    ...
    [Fact]
    public async Task Scaffold_DoubleNesting_NoExtraSourceDir()
    {
        var result = await _service.ScaffoldAsync(_tempDir, "Test", "DirRX");

        Assert.True(result.Success);
        // Correct: outputPath/DirRX.Test/DirRX.Test.Shared/ (double nesting)
        Assert.True(Directory.Exists(Path.Combine(_tempDir, "DirRX.Test", "DirRX.Test.Shared")));
        // Must NOT create extra source/ inside module dir
        Assert.False(Directory.Exists(Path.Combine(_tempDir, "DirRX.Test", "source")));
    }
